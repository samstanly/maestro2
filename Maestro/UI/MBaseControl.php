...
class MBaseControl extends MBase
{
    ...
    public function __get($property)
    {
        ...
        if ($value instanceof \Maestro\Utils\MNULL) {
    ...
    public function __set($property, $value)
    {
        ...
        if ($set instanceof \Maestro\Utils\MNULL) {
            $selector = MStyle::selector($property);
            if ($selector != "") {
    ...
    {
        $xmlControls = new \Maestro\UI\MXMLControls();
    ...
    function onCreate()
    {
        ...
        $this->result = '';
        $this->inner = '';
        $this->property->id = $this->className . '_' . substr(uniqid(), -6);
    ...
    public function addControl($control, $pos = NULL)
    {
        if (!is_null($control)) {
            if (is_array($control)) {
                ...
                $index = $control->property->id ? : uniqid();
                $this->controls[$index] = $control;
            } else {
                $this->controls[uniqid()] = $control;
            }
        }    
    }
    ...
    public function setData($data, $control = null)
    {
        ...
        if ($current->hasItems()) { // é um container: chamada recursiva
            ...
            if ($name) {
                ...
                if (isset($rawValue)) {
                    if ($rawValue instanceof \Maestro\Types\MCurrency) {
                        $value = $rawValue->getValue();
                    } else if ($rawValue instanceof \Maestro\Types\MCPF) {
                        $value = $rawValue->getPlainValue();
                    } else if ($rawValue instanceof \Maestro\Types\MCNPJ) {
                        $value = $rawValue->getPlainValue();
                    } elseif (($rawValue instanceof \Maestro\Types\MDate) || ($rawValue instanceof \Maestro\Types\MTimestamp)) {
